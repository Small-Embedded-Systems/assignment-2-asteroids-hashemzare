...
const colour_t background = rgb(2, 5, 7);

/** Function draws the game onto the LCD **/
...
void draw(void){
        ...
        lcd->printf("Total Score: %i", totaltimealive);
				...
				if(shieldhealth==2){
					/** Half-health shield **/
					lcd->drawLine(originX+leftX,originY+leftY,originX+pointX,originY+pointY,YELLOW);
					lcd->drawLine(originX+leftX,originY+leftY,originX+rightX,originY+rightY,YELLOW);
					lcd->drawLine(originX+rightX,originY+rightY,originX+pointX,originY+pointY,YELLOW);
				}
				if(shieldhealth==1){
					/** Low shield **/
...
void gameovertext(void){
	lcd->setTextSize(3);
	lcd->fillScreen(background);
	lcd->setCursor(150,105);
	lcd->printf("GAME OVER");
	lcd->setCursor(120,135);
